...
async def get_dataset_info():
    ...
    return {"success": False, "metadata": None, "message": "No dataset loaded"}
...
async def reset_to_default():
    """
    Reset to the default dataset by re-running the original pipeline.
    This is a convenience endpoint for development.
    """
    try:
        # Run the original dashboard generation script
        import subprocess
        
        script_path = os.path.join(PROJECT_ROOT, "python", "07_generate_dashboard_data.py")
        
        if not os.path.exists(script_path):
            return JSONResponse(
                status_code=404,
                content={"success": False, "message": "Original pipeline script not found."},
            )
        ...
        if result.returncode == 0:
            # Remove metadata file to indicate default dataset
            if os.path.exists(METADATA_FILE):
                os.remove(METADATA_FILE)
            
            return {"success": True, "message": "Reset to default dataset successful."}
        else:
            return JSONResponse(
                status_code=500,
                content={
